...
public class FogChartView extends View {
    ...
    public FogChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        int j = 0;
...
//      Y轴箭头
        ...
        if(data.size() > 1){
            for(int i=1; i<data.size(); i++){
                ...
                float startY = YPoint - m  / 100 * YScale;
                float n = (float)data.get(i);
                float stopY = YPoint - n / 100 * YScale;
